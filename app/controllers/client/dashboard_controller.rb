class Client::DashboardController < ApplicationController
  ...
  def index
    ...
    @monthly_batches = current_client.form_submission_batches
    ...
    # クリック集計
    # -------------------------
    click_links = ClickTrackingLink.where(client_id: current_client.id)

    @total_clicks = click_links.sum(:clicked_count)

    @clicked_users_count = click_links
                             .where.not(last_clicked_at: nil)
                             .count

    @click_rate =
      if @total_sent > 0
        ((@clicked_users_count.to_f / @total_sent) * 100).round(1)
      else
        0
      end
  end
