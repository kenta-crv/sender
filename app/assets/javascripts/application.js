...
(function() {
  ...
  const mountDataTargetNav = () => {
    // 重複追加を防ぐため、一度イベントを除去してから登録
    document.body.removeEventListener('click', handleDataTargetClick);
    document.body.addEventListener('click', handleDataTargetClick);
  };

  const handleDataTargetClick = (e) => {
  ...
  const initTrialOverlay = () => {
    const overlay = document.getElementById('trialOverlay');
    const closeBtn = document.getElementById('closeTrialModal');
    if (overlay && closeBtn) {
      overlay.classList.add('is-visible');
      closeBtn.onclick = (e) => { e.preventDefault(); overlay.classList.remove('is-visible'); };
      overlay.onclick = (e) => { if (e.target === overlay) overlay.classList.remove('is-visible'); };
    }
  };
