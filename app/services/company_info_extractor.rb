...
class CompanyInfoExtractor
  TEL_REGEX = /(?<!\d)0\d{1,4}-\d{1,4}-\d{3,4}(?!\d)/
  ...
  def clean_address(text)
    ...
    stop_pattern = /
      (?:TEL|Tel|tel|ＴＥＬ|℡|電話|FAX|Fax|fax|ＦＡＸ|
         ...
         代表者|設立|資本金|従業員|業務内容|事業内容|事業案内|施工計画|会社情報|
         代表挨拶|経営理念|行動指針|拠点一覧|地域社会への貢献|沿革|
         役員|取締役|営業本部|店[\s　]*舗|店舗|
         サイトマップ|個人情報保護方針|購入ページ|TOP|事業一覧|
    ...
    s = s.split(/[・、,]\s*(?:法人営業部|[一-龥ァ-ヶA-Za-z]{1,12}(?:支店|営業所|出張所|オフィス|事業所|工場|倉庫|センター))\s*〒\s*\d{3}-?\d{4}/, 2).first.to_s
    s = s.split(/(?<=[0-9０-９号番地])\s*(?:法人営業部|[一-龥ァ-ヶA-Za-z]{1,12}(?:支店|営業所|出張所|オフィス|事業所|工場|倉庫|センター))\s*〒\s*\d{3}-?\d{4}/, 2).first.to_s
